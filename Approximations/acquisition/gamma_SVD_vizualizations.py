...
file=open("Approximations/run data/svd/batch "+str(batch_id)+"/successful run data.txt","r")
...
run_data=np.array(run_data,float)
run_data=run_data[run_data[:, -2].argsort()]
cut_off_value=10
cut_off_point=np.min((np.sqrt(run_data[:,-2])>cut_off_value).nonzero())
print(cut_off_point,cut_off_point/run_data.shape[0])

neutron_gammas=run_data[:,]
